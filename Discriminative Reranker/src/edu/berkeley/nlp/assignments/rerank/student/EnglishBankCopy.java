...
public class EnglishBankCopy<L>
{
	abstract static class AbstractEval<L, O>
	{
		...
		public double getF1(Tree<L> guessT, Tree<L> goldT) {
			Set<O> guessedSet = makeObjects(guessT);
			Set<O> goldSet = makeObjects(goldT);
			Set<O> correctSet = new HashSet<O>();
			correctSet.addAll(goldSet);
			correctSet.retainAll(guessedSet);

			int correct = correctSet.size();
			int guess = guessedSet.size();
			int gold = goldSet.size();

			  //    System.out.println( guessT.toString() );
			  //    System.out.println( goldT.toString() );
			double precision = (guess > 0 ? correct / (double) guess : 1.0);
