...
public class HMMSolverLastState
{
	...
	StringIndexer frenchIndexer;
        ...
        public HMMSolverLastState(Iterable<SentencePair> trainingData, boolean useFirstAsEnglish, int numIter) throws Exception{

            this.numOfIterations = numIter;
            ...
            for(int x=0; x<maxEnglishSentenceLength; x++){
                ...
                hmmParamsOdd.lastStateProbs.put(x,priorInitVal);
                for(int y=0; y<maxEnglishSentenceLength; y++){
                    int tcode = Utils.getEncodingTransition(x, y);
                    hmmParamsOdd.transitionProbs.put(tcode, Math.log( 0.5/((Math.abs(x-y))+1.0) ) );                 
            ...
            // init hmm
            System.out.println("maxEnglishSentenceLength= "+maxEnglishSentenceLength);
            hmm = new HMMLastState(maxEnglishSentenceLength, efKeyset, maxFrenchSentenceLength);

            // training procedure
            train();
        }
	public HMMSolverLastState(Iterable<SentencePair> trainingData, boolean useFirstAsEnglish) throws Exception{
            this.englishIndexer = new StringIndexer();
            this.frenchIndexer = new StringIndexer();
            hmmParamsEven = new HMMParams();
            hmmParamsOdd = new HMMParams();
	...
	private void train() throws Exception{
        ...
        private void resetAll(HMMParams hmmParams) throws Exception{
            ...
            for(int i=0; i<numOfStates; i++){
                ...
                hmmParams.lastStateProbs.put(i,Double.NEGATIVE_INFINITY);                
            }
        }
	...
	private void performEM(int iterNum) throws Exception{
            ...
            normalize(timeSteps, numOfStates, estimateHmmParams);
            System.out.println("loglikelihood = "+logLikelihood);
            
        }

        
        private void normalize(int T, int numOfStates, HMMParams hmmParams ) throws Exception{
            
            IntCounter estimateTransitionProbs = hmmParams.transitionProbs, estimateEmmissionProbs=hmmParams.emmissionProbs, 
                 estimatePriorProbs=hmmParams.priorProbs, estimateLastStateProbs =hmmParams.lastStateProbs;
            ...
            priorSum=Double.NEGATIVE_INFINITY;
            for(int i=0;i<numOfStates;i++){
                priorSum = Utils.logAdd(priorSum, estimateLastStateProbs.get(i));
            }
            for(int i=0;i<numOfStates;i++){
                estimateLastStateProbs.incrementCount(i, -priorSum);
                if(false && i<10) System.out.println("prior " + i + " " + Math.exp(estimateLastStateProbs.get(i)));
            }  
