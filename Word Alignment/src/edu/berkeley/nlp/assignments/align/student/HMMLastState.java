...
public class HMMLastState {
	...
	private int efKeyset[];
        ...
        public HMMLastState(int maxNumberOfStates, int[] efKeyset, int maxNumberOfTimeSteps){
    ...
    double computForwardScores(int[] curEnglish, int[] curFrench, HMMParams curhmmparams, int[] efKeyset) throws Exception {
        ...
        IntCounter lastStateProbs = curhmmparams.priorProbs;
            ...
            for(int s=0; s<numOfStates;s++){
                    ret = Utils.logAdd(ret,alpha[s][T-1]+lastStateProbs.get(numOfStates-1-s));
            }
            if(Utils.debug){
    ...
    double computBackwardScores(int[] curEnglish, int[] curFrench, HMMParams curhmmparams, int[] efKeyset) throws Exception {
        ...
        int T=curFrench.length;
        int numOfStates = curEnglish.length;
        for(int s=0; s<numOfStates;s++) beta[s][T-1]= lastStateProbs.get(numOfStates-1-s); // log(1) = 0
        for(int t=T-2;t>=0;t--){
                for(int s=0; s<numOfStates;s++){
    ...
    void computeAndUpdatePrior(int[] curEnglish, int[] curFrench, 
        ...
        //last state probs ...
        prior = new double[numOfStates];
        for(int i=0; i<numOfStates;i++){
            tmp =  alpha[i][T-1] + curhmmparams.lastStateProbs.get(numOfStates-1-i)- sentProb;
            double prev = estimateParams.lastStateProbs.get(numOfStates-1-i);
            estimateParams.lastStateProbs.put(numOfStates-1-i, Utils.logAdd(prev, tmp) );
        }
    }
    ...
    void computeAndUpdateEmmission(int[] curEnglish, int[] curFrench, 
		...
		for(int s=0; s<numOfStates; s++){
			if((dp[s][T-1]+curhmmparams.lastStateProbs.get(numOfStates-1-s))>maxVal){
				maxVal=dp[s][T-1] +curhmmparams.lastStateProbs.get(numOfStates-1-s) ;
				maxValIdx=s;
			}
		}
